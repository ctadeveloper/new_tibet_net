 ...
 */
function adobePDF_script(){
    $the_theme     = wp_get_theme();
    $theme_version = $the_theme->get('Version');
    if (has_post_format(array('aside','chat'))) {
    // js
    $js_version = $theme_version . '.' . filemtime(get_template_directory() . '/js/adobePDF/adobePdf.min.js');
    wp_enqueue_script('adobePDF-js', get_template_directory_uri() . '/js/adobePDF/adobePdf.min.js', array(), $js_version, true);
    }
}
add_action('wp_enqueue_scripts', 'adobePDF_script');
